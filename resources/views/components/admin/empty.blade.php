...
<div class="px-6 py-16">
    <span class="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-brand-50 text-brand-600 ring-1 ring-brand-200">
        <x-icon :name="$icon" class="w-6 h-6" />
    </span>
    <h3 class="mt-4 text-base font-semibold text-slate-900">{{ $title }}</h3>
    @if ($message)<p class="mt-1 max-w-md text-sm text-slate-500">{{ $message }}</p>@endif
